#!/usr/bin/env python3
"""
Generate audio responses from the DISCRETE-LABEL ablation model for empathy evaluation.

Ablation counterpart of generate_responses_emotional_sympatheia.py, following the
SAME competitor-model convention as generate_responses_emotional_glm4voice.py /
_opens2s.py / _kimiaudio.py: it READS the original VA manifest.jsonl (produced by
generate_responses_emotional_sympatheia.py), reuses the exact same query audio for
a fair comparison, and writes a SEPARATE manifest_discrete.jsonl. Nothing in the
...
Conditions produced:
  ...
  finetuned_discrete_na — discrete-label checkpoint + plain prompt (no emotion cue),
...
Usage:
    python -m eval.generate_responses.sympatheia_emotional.generate_responses_emotional_discrete \\
...
# Emotional setting's no-cue baseline is a plain prompt (matches the VA script's
# finetuned_na in generate_responses_emotional_sympatheia.py).
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate discrete-label GLM-4-Voice responses for empathy evaluation "
                    "(reads the original VA manifest, writes manifest_discrete.jsonl)"
    )
    parser.add_argument(
        "--manifest", type=str, required=True,
        help="Path to the ORIGINAL VA manifest.jsonl from "
             "generate_responses_emotional_sympatheia.py (used to select the same query audio files). "
    ...
    if todo:
        ...
        for idx, rec in enumerate(todo):
            ...
            prompts = {
                ...
                "finetuned_discrete_na": PLAIN_SYSTEM_PROMPT,
            }
